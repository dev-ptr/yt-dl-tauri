...
impl BinaryManager {
    ...
    pub async fn download_ffmpeg(app_handle: &tauri::AppHandle) -> Result<(), String> {
        ...
        let dest_path = Self::get_ffmpeg_path(app_handle)?;
        ...
        // For Windows/Linux, we need to extract from archive
        if cfg!(target_os = "windows") {
            Self::extract_ffmpeg_windows(&bytes, &dest_path)?;
        } else if cfg!(target_os = "linux") {
            Self::extract_ffmpeg_linux(&bytes, &dest_path)?;
        } else if cfg!(target_os = "macos") {
            Self::extract_ffmpeg_macos(&bytes, &dest_path)?;
        }

        Self::make_executable(&dest_path)?;

        println!("ffmpeg downloaded successfully to: {}", dest_path.display());
        Ok(())
    }

    #[cfg(target_os = "windows")]
    fn extract_ffmpeg_windows(bytes: &[u8], dest_path: &PathBuf) -> Result<(), String> {
        use std::io::Cursor;
        use zip::ZipArchive;
    ...
    fn extract_ffmpeg_windows(_bytes: &[u8], _dest_path: &PathBuf) -> Result<(), String> {
        Err("Windows extraction not supported on this platform".to_string())
    }

    #[cfg(target_os = "linux")]
    fn extract_ffmpeg_linux(bytes: &[u8], dest_path: &PathBuf) -> Result<(), String> {
        use std::io::Cursor;
        use tar::Archive;
    ...
    fn extract_ffmpeg_linux(_bytes: &[u8], _dest_path: &PathBuf) -> Result<(), String> {
        Err("Linux extraction not supported on this platform".to_string())
    }

    #[cfg(target_os = "macos")]
    fn extract_ffmpeg_macos(bytes: &[u8], dest_path: &PathBuf) -> Result<(), String> {
        use std::io::Cursor;
        use zip::ZipArchive;
        ...
        for i in 0..archive.len() {
            ...
            if file.name().ends_with("ffmpeg") && !file.name().contains('/') {
                let mut buffer = Vec::new();
                std::io::copy(&mut file, &mut buffer)
                    .map_err(|e| format!("Failed to extract ffmpeg: {}", e))?;
        ...
        Err("ffmpeg not found in archive".to_string())
    }

    #[cfg(not(target_os = "macos"))]
    fn extract_ffmpeg_macos(_bytes: &[u8], _dest_path: &PathBuf) -> Result<(), String> {
